...
export class AllProductListingsComponent implements OnInit {
  ...
  @ViewChild('relativeHeaderMobile', { static: true })
  relativeHeaderMobile?: HeaderMobileComponent;
  @ViewChild('fixedHeader', { static: true }) fixedHeader?: HeaderComponent;
  @ViewChild('fixedHeaderMobile', { static: true })
  fixedHeaderMobile?: HeaderMobileComponent;
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      this.http
        ...
        .subscribe((res: any) => {
          this.productsToDisplay = res.results.reverse();
  ...
  sortResults(results: ProductData[]) {
    switch (this.sortingService.getSortingMethod()) {
      case Sort.AtoZ:
        return results.sort((productA, productB) =>
      ...
      case Sort.priceAsc:
        return results.sort(
          (productA, productB) => productA.usd - productB.usd
        );
      case Sort.priceDesc:
        return results.sort(
          (productA, productB) => productB.usd - productA.usd
        );
      case Sort.newest:
        return results.sort((productA, productB) =>
          productB.createdAt.localeCompare(productA.createdAt)
        );
      case Sort.oldest:
        return results.sort((productA, productB) =>
          productA.createdAt.localeCompare(productB.createdAt)
        );
    }
  }
  ...
  setSortingMethod(e: any) {
    ...
    this.activeSort =
      sortEnumToSortText[this.sortingService.getSortingMethod()];
    this.productsToDisplay = this.sortResults(this.productsToDisplay);
  }

  addToCart(e: Event, productId: number, usdPrice: number) {
    e.stopPropagation();
    this.http
      .post('/api/addToCart', {
...
export const allCategories = ['T-shirts', 'Shirts', 'Pants', 'Shorts'];
